...
// Sample assignment info — replace with real data from the assigner's letter later
const SAMPLE_INSTRUCTOR = "Md. Kamal Hossain";
const SAMPLE_DURATION = "4 Weeks";

export default function TraineeProfile({ onBackHome, onLogout, initialSection = "profile" }) {
  ...
  useEffect(() => {
    const savedProfile = localStorage.getItem("agni_trainee_profile");
  ...
  const saveEdit = () => {
    setProfile(editForm);
    localStorage.setItem("agni_trainee_profile", JSON.stringify(editForm));
    setEditMode(false);
  };
        ...
        <main style={styles.main}>
            ...
            <div style={styles.profileWrap}>
                ...
                <div style={styles.bannerLeft}>
                  ...
                  <div>
                    <div style={styles.bannerName}>
                      {profile?.name || "Trainee Name Not Found"}
                    </div>
              ...
              {!profile ? (
                <p style={styles.noProfileNote}>
                  No trainee registration found on this device. Please
                  complete the Training Request form first.
                </p>
              ) : (
                <>
                  <div style={styles.infoFormGrid}>
                    ...
                    <InfoField
                      label="Degree"
                      name="degree"
                      value={editMode ? editForm.degree : profile.degree}
                      editable={editMode}
                      onChange={updateEditField}
                    />
                    <InfoField
                    ...
                    <InfoField
                      label="Experience"
                      name="experience"
                      value={editMode ? editForm.experience : profile.experience}
                      editable={editMode}
                      onChange={updateEditField}
                    />
                    <InfoField
                      label="Training ID"
                      name="trainingId"
                      value={profile.trainingId}
                      editable={false}
                      onChange={updateEditField}
                    />
                  </div>

                  {/* TRAINING ASSIGNMENT — locked, assigned by admin later */}
                  <h2 style={{ ...styles.sectionHeading, marginTop: "36px" }}>
                    Training Assignment
                  </h2>
                  <div style={styles.infoFormGrid}>
                    <InfoField
                      label="Instructor Name"
                      value={SAMPLE_INSTRUCTOR}
                      editable={false}
                    />
                    <InfoField
                      label="Duration"
          ...
          {section === "history" && (
            <div style={{ ...styles.card, ...styles.wideCard }}>
              ...
              <p style={styles.subLabel}>
                Showing all complaints submitted by citizens and trainees.
              </p>
              {complaints.length === 0 ? (
                <p style={styles.subLabel}>No complaints submitted yet.</p>
              ) : (
